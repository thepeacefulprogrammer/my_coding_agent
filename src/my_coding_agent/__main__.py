...
Examples:
  ...
  %(prog)s --font-size 14           # Start with larger font
    ...
    parser.add_argument(
        "--font-family",
        help="Font family for code display (default: from settings)"
    )
...
def _parse_window_size(size_str: str) -> tuple[int, int]:
    ...
    try:
        ...
        raise ValueError(f"Invalid window size format '{size_str}'. Use format like '1200x800'") from e
...
def validate_directory(directory: Path) -> Path:
    ...
    try:
        ...
        try:
            ...
            print(f"Error: Permission denied accessing directory: {directory}", file=sys.stderr)
            sys.exit(1)
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    print(f"My Coding Agent - Simple Code Viewer")
